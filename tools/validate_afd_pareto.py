...
def write_side_by_side_svg(path: Path, rows: list[dict]):
    ...
    for link_us in sorted({float(row["link_us"]) for row in rows}):
        ...
        nv_des_frontier = pareto_uplr([
            {
                "x": float(row["nanovllm_des_interactivity"]),
                "y": float(row["nanovllm_des_tok_s_per_gpu"]),
                "row": row,
            }
            for row in des_group
        ...
        nv_des_points = " ".join(f'{sx(right_x0, p["x"]):.2f},{sy(p["y"]):.2f}' for p in nv_des_frontier)
        if nv_des_points:
...
def write_link_overlay_svg(
    ...
    if colocated_nanovllm_des:
        body.append(polyline(colocated_nanovllm_des, "#111827", "4 2 1 2"))
...
def colocated_curve(rows: list[dict], case: str) -> list[dict]:
    aliases = {
        "des": {"des", "des_batched"},
        "direct": {"direct"},
        "nanovllm_des": {"nanovllm_des", "nano-vllm-des"},
    }
...
def build_colocated_rows(args) -> list[dict]:
    ...
    for point in direct_points:
        ...
        nv_des_x, nv_des_y, nv_des_tbt_ms = run_nanovllm_des_colocated_point(point, args)
        rows.append({
            "case": "nanovllm_des",
            "B": int(point["B"]),
            "tp_g": int(point["tp_g"]),
            "interactivity": nv_des_x,
            "tok_s_per_gpu": nv_des_y,
            "tpot_or_tbt_ms": nv_des_tbt_ms,
            "rel_x_err_vs_analytical": (nv_des_x - float(point["x"])) / float(point["x"]),
            "rel_y_err_vs_analytical": (nv_des_y - float(point["y"])) / float(point["y"]),
        })
    return rows


def run_nanovllm_des_colocated_point(point: dict, args) -> tuple[float, float, float]:
    trace_path = args.output_dir / "_nanovllm_des_colocated_tmp.csv"
    ...
    llm = LLM(
        ...
        max_num_batched_tokens=max(batch_size * (isl + 1), 1),
        max_model_len=isl + 2,
        mock_kv_capacity_tokens=batch_size * (isl + 2),
    ...
    if not token_times or not start_times:
        raise ValueError("nano-vLLM-DES trace did not contain a complete colocated decode block")
...
def open_triangle(x: float, y: float, color: str, r: float) -> str:
    return (
        f'<polygon points="{x:.2f},{y - r:.2f} {x + r:.2f},{y + r:.2f} '
        f'{x - r:.2f},{y + r:.2f}" fill="white" stroke="{color}" stroke-width="1.5"/>'
    )
...
def run_nanovllm_des_point(point: dict, args) -> tuple[float, float, float]:
    trace_path = args.output_dir / "_nanovllm_des_afd_tmp.csv"
    if trace_path.exists():
        trace_path.unlink()
    Sequence.counter = count()
    gb = int(point["gb"])
    ...
    llm = LLM(
        ...
        max_num_batched_tokens=max(gb * (isl + 1), 1),
        max_model_len=isl + 2,
        mock_kv_capacity_tokens=gb * (isl + 2),
        mock_block_size=isl + 2,
        prefill_base_ms=0.0,
        prefill_ms_per_token=0.0,
        microbatch_size=ck,
...
def compare_rows(points: list[dict], args) -> list[dict]:
    ...
    for idx, point in enumerate(points, start=1):
        ...
        rows.append({
            ...
            "nanovllm_des_interactivity": nv_des_x,
